...
public class SqlMetaDataBo {
    ...
    public void readRowKey(byte[] rowKey) {
        this.agentId = BytesUtils.safeTrim(BytesUtils.toString(rowKey, 0, PinpointConstants.MAX_LEN));
    ...
    private static long readTime(byte[] rowKey) {
        return BytesUtils.bytesToLong(rowKey, PinpointConstants.MAX_LEN);
    }

    private static int readSqlId(byte[] rowKey) {
        return BytesUtils.bytesToInt(rowKey, PinpointConstants.MAX_LEN + BytesUtils.LONG_BYTE_LENGTH);
    }
