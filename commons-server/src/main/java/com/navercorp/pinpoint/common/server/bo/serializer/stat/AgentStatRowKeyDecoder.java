...
public class AgentStatRowKeyDecoder implements RowKeyDecoder<AgentStatRowKeyComponent> {
    ...
    public AgentStatRowKeyComponent decodeRowKey(byte[] rowkey) {
        final String agentId = BytesUtils.safeTrim(BytesUtils.toString(rowkey, 0, MAX_LEN));
        final AgentStatType agentStatType = AgentStatType.fromTypeCode(rowkey[MAX_LEN]);
        final long reversedBaseTimestamp = BytesUtils.bytesToLong(rowkey, MAX_LEN + TYPE_CODE_BYTE_LENGTH);
